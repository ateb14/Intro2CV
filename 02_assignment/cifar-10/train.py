...
# https://developer.apple.com/forums/thread/719225
# Something important to use tensorboard in MacOS M1
# Also we should reinstall protobuf==3.20.3 manually
# Apple, do something!!!
mps_device = torch.device("mps")


def MyCELoss(pred, gt):
    # pred \in R^{B, C}, where B is batch size, C is number of classes
    # gt \in R^{B}, where B is batch size

    # calculate cross entropy loss
    loss = -torch.log(pred[torch.arange(pred.shape[0]), gt])
    loss = loss.sum()
    return loss
...
def train(epoch, model, optimizer, train_loader, writer):
    ...
    for imgs, labels in tqdm.tqdm(train_loader):
        ...
        if iteration % 50 == 0:
            ...
            writer.add_scalar('train/acc1', top1.val, iteration)
            writer.add_scalar('train/acc5', top5.val, iteration)
    ...
    for epoch in range(start_epoch, args.total_epoch):
        ...
        if epoch % args.save_freq == 0:
            ...
            save_file = os.path.join(
                ckpt_folder, 'ckpt_epoch_%s' % (str(epoch)))
