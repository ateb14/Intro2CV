...
def compute_segmentation_ap(output_list, gt_labels_list, iou_threshold=0.5):
    ...
    for cls in classes:  # per class app
        ...
        for img_idx, pred_labels in enumerate(output_list):  # different images
            # different objects detected in the same image
            for obj_idx, pred_label in enumerate(pred_labels['labels']):
                ...
                if pred_label == cls:
                    ...
                    if gt['labels'][0] == cls:
                        ...
                        pred_mask = pred_labels['masks'][obj_idx].detach(
                        ).numpy().reshape(gt_mask.shape)
        ...
        recall_to_precision = sorted(
            recall_to_precision.items(), key=lambda x: x[0])
            ...
            for obj_idx, pred_label in enumerate(pred_labels['labels']):
                ...
                if pred_label == cls:
                    ...
                    if gt['labels'][0] == cls:
                        ...
                        # compute union area
                        gt_area = gt['area'][0]
                        pred_area = (pred_box[3] - pred_box[1]) * \
...
model.load_state_dict(torch.load(
    r'E:\Intro2CV\04_assignment\MaskRCNN\results\maskrcnn_2.pth', map_location='cuda'))
