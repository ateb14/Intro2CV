...
class ShapeDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        bg_color = np.asarray([random.randint(0, 255) for _ in range(3)]) # background color
        ...
        for i in range(n_shapes):
            while 1: # keep drawing shapes until overlap is small enough
                ...
                if i == 0:
                    ...
                    remaining_shape_percentage = np.sum(masks[i, :]) / current_shape_area
        ...
        boxes = torch.as_tensor(boxes, dtype=torch.float32)
        labels = torch.as_tensor(shape_codes, dtype=torch.int64)
        masks = torch.as_tensor(masks, dtype=torch.uint8)  
        area = (boxes[:, 3] - boxes[:, 1]) * (boxes[:, 2] - boxes[:, 0])
